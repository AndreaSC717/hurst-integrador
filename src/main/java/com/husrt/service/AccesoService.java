...
public class AccesoService {
    ...
    public ResultadoIngreso intentarIngresoEstudiante(String cedula) throws SQLException {
        ...
        if (!estudiantes.existsInscripcionActiva(idEst, anio, periodo)) {
            return rechazo(idEst, null, "Sin inscripción activa para el semestre actual.");
        }
        if (!e.induccionCompletada()) {
            alertas.insert("INDUCCION", idEst, null, "Intento de ingreso sin inducción.");
        ...
        if (e.arlInicio() == null || e.arlFin() == null || hoy.isBefore(e.arlInicio()) || hoy.isAfter(e.arlFin())) {
            alertas.insert("ARL", idEst, null, "Intento de ingreso con ARL no vigente.");
            return rechazo(idEst, null, "ARL no vigente a la fecha de ingreso.");
        }

        Optional<AsignacionPractica> asig = planes.findAsignacionValidaIngreso(idEst, anio, periodo, mes, hoy, diaSemanaIso, ahora);
        if (asig.isEmpty()) {
            alertas.insert("FRANJA", idEst, null, "Ingreso fuera de franja o sin plan para el mes actual.");
            return rechazo(idEst, null, "No existe franja horaria habilitada en el plan de prácticas para este momento.");
        }
        ...
        if (!regDocentes.tieneEntradaAbiertaParaPlan(plan.idDocente(), plan.idPlan())) {
            alertas.insert("DOCENTE", idEst, plan.idDocente(), "Intento de ingreso sin docente responsable dentro del hospital para el plan.");
        ...
        if (registros.tieneEntradaAbierta(idEst)) {
            return rechazo(idEst, a.idAsignacion(), "El estudiante ya tiene una entrada activa sin salida.");
        }
        ...
        if (asignados >= srv.get().capacidadMaximaEstudiantes()) {
            alertas.insert("CAPACIDAD_PLAN", idEst, null,
                    "El plan asignó el máximo de estudiantes (" + asignados + ") para " + srv.get().nombre() + " en esta franja.");
        }
        if (dentro >= srv.get().capacidadMaximaEstudiantes()) {
            alertas.insert("CAPACIDAD", idEst, null, "Servicio saturado con estudiantes dentro.");
            return rechazo(idEst, a.idAsignacion(), "Servicio saturado: ya hay el máximo de estudiantes dentro.");
        }
    ...
    public String registrarSalidaEstudiante(String cedula) throws SQLException {
        ...
        if (idReg.isEmpty()) {
            return "No hay entrada activa para registrar salida.";
        }
    ...
    public String docenteEntrada(String cedulaDocente, long idPlan) throws SQLException {
        ...
        if (p.get().idDocente() != d.get().idDocente()) {
            alertas.insert("DOCENTE_PLAN", null, d.get().idDocente(), "Docente intentó check-in en plan ajeno.");
        ...
        if (regDocentes.tieneEntradaAbiertaParaPlan(d.get().idDocente(), idPlan)) {
            return "El docente ya tiene ingreso activo para este plan.";
        }
    ...
    public String docenteSalida(String cedulaDocente, Long idPlan) throws SQLException {
        ...
        if (idR.isEmpty()) {
            return "No hay ingreso activo del docente para cerrar.";
        }
